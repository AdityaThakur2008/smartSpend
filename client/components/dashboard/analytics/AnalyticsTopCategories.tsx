...
export default function AnalyticsTopCategories({ data }: AnalyticsTopCategoriesProps) {
  
  const totalAmount = data.reduce((sum, item) => sum + Number(item.totalAmount), 0);

  const sortedData = [...data].sort((a, b) => b.totalAmount - a.totalAmount).slice(0, 5); 
  return (
    <div className="bg-card border border-border/50 rounded-2xl p-6 shadow-sm flex flex-col">
      <h3 className="font-bold text-foreground mb-6">Top Spending Categories</h3>
